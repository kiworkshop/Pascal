...
export default class SearchManager {
  ...
  search(payload, exactness = true) {
    let searchingProducts = this.trimSearchingProducts(
  ...
  trimSearchingProducts(searchingProducts) {
    return Object.keys(
      searchingProducts.split(/[,]+[\s]*/).reduce((accumulator, value) => {
        if (!accumulator.hasOwnProperty(value) && Boolean(value)) {
          // 상품 이름이 공란인 경우는 추가 대상에서 제외
  ...
  makeURL(searchingProducts) {
    ...
    const baseURL =
      "https://oow1cmv2z7.execute-api.ap-northeast-2.amazonaws.com/Pascal";
    const encodedURL =
      baseURL + "?format=json&pretty=true&q=" + encodeURI(productsString);
    return encodedURL;
  }
  ...
  makeRequests(_class, searchingProducts, exactness) {
    ...
    for (let cycle = 0; cycle < repeat; cycle++) {
      requests.push(
        this.makeRequestPromise(
          this.makeURL(
            searchingProducts.slice(batchSize * cycle, batchSize * (cycle + 1))
          )
        )
      );
    }
    ...
    return Promise.all(requests).then(response => {
      ...
      if (exactness) {
        return this.classifyNoticedOrUnnoticed(
          _class,
          searchingProducts,
          searchResult
        );
  ...
  makeRequestPromise(url) {
    return axios.get(url).then(response => {
      ...
      let noticedProductSearcher = axios.get(
        url + "&size=10000&cursor=initial"
      );
      for (let cycle = 0; cycle < repeatTimes - 1; cycle++) {
        //10000개 단위로 결과를 받아올 때, 순차적으로 query를 보내도록 함
  ...
  classifyNoticedOrUnnoticed(_class, searchingProducts, checklist) {
    return searchingProducts.reduce(
      (accumulator, searchingProduct) => {
